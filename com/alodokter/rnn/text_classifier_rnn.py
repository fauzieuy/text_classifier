...
class TextClassifierRNN:
    def __init__(self, sequence_length, num_classes, vocab_size, embedding_size, l2_reg_lambda=0.0):
        ...
        with tf.device('/cpu:0'), tf.name_scope("embedding"):
            ...
            # returned shape -> [None, sequence_length, embedding_size]
            embedded_words = tf.nn.embedding_lookup(W_embedding, self.input_x)
            embedded_words_list = tf.unstack(embedded_words, axis=1)
        ...
        # and passes embedded_words_list as inputs for each unit.
        initial_state = cell.zero_state(self.batch_size, tf.float32)
        outputs, state = tf.contrib.rnn.static_rnn(cell, embedded_words_list, initial_state=initial_state)
        ...
        with tf.name_scope("output"):
            ...
            # using rnn inner loop last output
